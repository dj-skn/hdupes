...
{
#ifndef NO_HELPTEXT
  printf("Usage: hdupes [options] FILES and/or DIRECTORIES...\n\n");
...
#endif /* NO_JSON */
#ifdef HDUPES_HAS_THREADS
  printf(" -J --threads=#   \tpre-hash files using N threads (hashing stage only)\n");
#endif
/*  printf(" -K --skip-hash   \tskip full file hashing (may be faster; 100%% safe)\n");
  ...
  printf(" -v --version     \tdisplay hdupes version and build information\n");
#ifndef NO_EXTFILTER
  ...
  printf("                  \tPassing '-y .' will expand to  '-y hdupes_hashdb.txt'\n");
...
void version_text(int short_version)
{
  ...
  if (!short_version) {
    printf(", libjodycode %s (%s)\n", jc_version, jc_verdate);
    printf("Hashes:");
    for (int i = 0; i < HASH_ALGO_COUNT; i++)
      printf(" %s%c", hash_algo_list[i], i == (HASH_ALGO_COUNT - 1) ? '\n' : ',');
  } else {
    printf("\n");
  }
