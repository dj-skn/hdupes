...
# hdupes output (known as "printmatches") to perform custom actions.
# Use it like this:
#
# hdupes whatever_parameters_you_like | ./example.sh script_parameters
#
# If you are on Windows, hdupes uses backslash path separators which
# must be converted to forward slashes before piping to this script,
# and carriage returns (\r) must also be deleted from hdupes output
# (tested on MSYS2 MinGW, probably true for similar environments):
#
# hdupes params | tr '\\' / | tr -d '\r' | ./example.sh script_params
#
# The general structure of hdupes pipe scripts are:
...
# consisting of duplicates that 'hdupes -rdN' would delete.

# Announce what this script does so the user knows what's going on
echo "hdupes example script - moving duplicate files to a directory"
