/* hdupes duplicate file finder utility
  ...
  while ((opt = GETOPT(argc, argv, GETOPT_STRING
    ...
    case 'J':
#ifdef HDUPES_HAS_THREADS
      {
        ...
        if (value > 256) {
          fprintf(stderr, "warning: --threads capped at 256; using 256\n");
          value = 256;
        }
...
#else
      fprintf(stderr, "warning: --threads is not supported in this build\n");
#endif
      break;
    case 'K':
    ...
    case 'y':
      ...
      if (strcmp(optarg, ".") == 0) strcpy(hashdb_name, "hdupes_hashdb.txt");
    ...
    default:
      ...
      fprintf(stderr, "Try `hdupes --help' for more information.\n");
      exit(EXIT_FAILURE);
    }
  }
...
/* We don't need the double traversal check tree anymore */
  ...
  if (hash_threads > 1) {
    if (!ISFLAG(flags, F_HIDEPROGRESS))
      fprintf(stderr, "Pre-hashing files using %u threads...\n", hash_threads);
    prehash_files(files, hash_threads);
    if (!ISFLAG(flags, F_HIDEPROGRESS))
      fprintf(stderr, "Pre-hashing complete.\n");
  }
#endif
