...
static int write_hashdb_entry(FILE *db, hashdb_t *cur, uint64_t *cnt, const int destroy)
{
  ...
  if (unlikely(cur == NULL)) {
    ...
    snprintf(out, PATHBUF_SIZE + 127, "hdupes hashdb:%d,%d,%08lx\n", HASHDB_VER, hash_algo, (unsigned long)tm.tv_sec);
...
/* db header format: hdupes hashdb:dbversion,hashtype,update_mtime
...
int64_t load_hash_database(const char * const restrict dbname)
{
  ...
  field = strtok(buf, ":");
  if (strcmp(field, "hdupes hashdb") != 0) goto error_hashdb_header;
  field = strtok(NULL, ":");
  temp = strtok(field, ",");
...
warn_hashdb_open:
  ...
  new_hashdb = 1;
  if (db != NULL) fclose(db);  /* fopen failure also lands here; glibc fclose(NULL) crashes */
  return 0;
error_hashdb_read:
...
static int get_path_hash(char *path, uint64_t *path_hash)
{
  ...
  *path_hash = 0;
  if ((uintptr_t)path & 0x0f) {
    ...
    retval = jc_block_hash(NORMAL, (jodyhash_t *)aligned_path, path_hash, strlen((char *)aligned_path));
  } else retval = jc_block_hash(NORMAL, (jodyhash_t *)path, path_hash, strlen(path));
  return retval;
}
