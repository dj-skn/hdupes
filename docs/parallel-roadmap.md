...
- Phase 4: CLI wiring for threaded pipeline with `--legacy-tree` fallback -
  implemented
...
Threaded scans now avoid hashing unique-size files. The threaded hash stage
does this work:
...
Large matched pairs are confirmed with a bounded `pread()` worker pool when
`--threads` is active. Small pairs and non-threaded scans use the serial
