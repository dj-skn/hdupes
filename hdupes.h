...
#if defined _WIN32 || defined __MINGW32__
 typedef uint64_t hdupes_ino_t;
 typedef uint32_t hdupes_mode_t;

#else /* Not Windows */
 #include <sys/stat.h>
 typedef ino_t hdupes_ino_t;
 typedef mode_t hdupes_mode_t;
 #ifdef UNICODE
...
#if !defined(ON_WINDOWS) && !defined(NO_THREADS)
 #define HDUPES_HAS_THREADS 1
#endif
...
#define F_SORTGROUPS		(1ULL << 20)
#define F_EXPERIMENTALPIPELINE	(1ULL << 21)
#define F_LEGACYTREE		(1ULL << 22)
